...
common::utils::Circuit<Ring> generateDataChangeCircuit(int nP, int pid, size_t num_verts, size_t num_edges, size_t num_changes) {
    ...
    for (size_t c = 0; c < num_changes; ++c) {
        std::cout << "Processing change " << (c+1) << "/" << num_changes << std::endl;
...
void benchmark(const bpo::variables_map& opts) {
    ...
    auto num_changes = opts["num-changes"].as<size_t>();
...
bpo::options_description programOptions() {
    ...
    desc.add_options()
        ...
        ("num-changes", bpo::value<size_t>()->required(), "Number of data changes to apply.")
