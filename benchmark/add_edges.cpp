...
common::utils::Circuit<Ring> generateCircuit(int nP, int pid, DistributedDaglist dist_daglist) {
    ...
    for (int i = 0; i < nC; ++i) {
        ...
        auto [prop_out_key, prop_out_v] = 
            circ.addGroupwisePropagateGate(IOut[i], data_e_out, new_edge_src_group[i], permutation[i]);
    ...
    for (int i = 0; i < nC; ++i) {
        ...
        std::vector<wire_t> subg_new_edge_sigd_values(add_ESizes[i]);
        auto [prop_out_key, prop_out_v] = 
            circ.addGroupwisePropagateGate(IIn[i], data_e_in, new_edge_dest_group[i], permutation[i]);
        auto [out_ind, ind_output_key, ind_output_v] = 
    ...
    auto prop_in = addSubCircPropagate(circ, sigd, Gin, nV, permutation2, true);

    // Reorder sigv to destination order
    auto sigv_d = addSubCircPermList(circ, sigd, {sigv}, permutation2)[0];
